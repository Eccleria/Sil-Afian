...
const action = async (interaction) => {
  ...
  const personality = PERSONALITY.getPersonality(); //get personality
  ...
  if (message.attachments.size !== 0) {
    ...
    await channelSend(logChannel, { files: attachments });
...
const saveLog = {
  ...
  command,
  help: (interaction) => {
    const personality = PERSONALITY.getPersonality().saveLog;
